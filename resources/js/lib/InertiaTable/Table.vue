<template>
    <fieldset ref="tableFieldset" :key="`table-${name}`" :dusk="`table-${name}`" class="min-w-0" :class="{ 'opacity-75': isVisiting }">
        <div class="flex flex-row flex-wrap justify-start px-4 sm:flex-nowrap sm:px-0">
            <slot
                name="tableFilter"
            ...
            <div v-if="queryBuilderProps.globalSearch" class="order-1 mb-2 flex w-full flex-row sm:order-2 sm:mb-0 sm:mr-4 sm:w-auto sm:grow">
                <slot
            ...
            <slot name="tableReset" can-be-reset="canBeReset" :on-click="resetQuery">
                <div v-if="canBeReset" class="order-5 ml-auto sm:order-3 sm:mr-4">
            ...
            >
                <TableAddSearchRow
                    ...
                    class="order-3 mr-2 sm:order-4 sm:mr-4"
                    :search-inputs="queryBuilderProps.searchInputsWithoutGlobal"
                    :has-search-inputs-without-value="queryBuilderProps.hasSearchInputsWithoutValue"
                    :on-add="showSearchInput"
                />
            ...
            >
                <TableColumns
                    ...
                    class="order-4 mr-4 sm:order-5 sm:mr-0"
                    :columns="queryBuilderProps.columns"
                    :has-hidden-columns="queryBuilderProps.hasHiddenColumns"
                    :on-change="changeColumnStatus"
                />
                            ...
                            <tbody class="divide-y divide-border">
                                <slot name="body" :show="show">
                                    ...
                                    >
                                        ...
                                        >
                                            <slot :key="key" :name="`cell(${column.key})`" :item="item" :key-from-the-end="resourceData.length - key">
                                                {{ item[column.key] }}
                                            </slot>
                                        </td>
                                    </tr>
...
const queryBuilderProps = computed(() => {
    let data = page.props.queryBuilderProps ? page.props.queryBuilderProps[props.name] || {} : {};

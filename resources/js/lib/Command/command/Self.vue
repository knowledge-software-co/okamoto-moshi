    ...
    <template #footer>
      ...
      <ul class="command-palette-commands">
        <li>
          ...
          ><span class="command-palette-Label">選択</span>
        </li>
        <li>
          <kbd class="command-palette-commands-key"
          ...
          ><span class="command-palette-Label">移動</span>
        </li>
        <li>
          <kbd class="command-palette-commands-key"
          ...
          ><span class="command-palette-Label">閉じる</span>
        </li>
      </ul>
    </template>
  </Command.Dialog>
...
const handleSelectTheme = (item: ThemeItem) => {
  activePage.value = item.value
  emit('select', item.value)
}
onClickOutside(el, (event: Event) => {
  const target = event?.target as HTMLElement;
  if (target?.getAttribute('command-dialog-mask') !== null) {
    emit("dialog", false);
  }
});
</script>

...
namespace App\Traits\InertiaTable;
...
class InertiaTable
{
    ...
    public function __construct(Request $request)
    {
        ...
        if (self::$defaultGlobalSearch !== false) {
            $this->withGlobalSearch(self::$defaultGlobalSearch);
        }
    }
    ...
    {
        self::$defaultGlobalSearch = $label !== false ? __($label) : false;
    }
    ...
    {
        if (empty(self::$defaultQueryBuilderConfig)) {
            self::$defaultQueryBuilderConfig = config('query-builder.parameters');
        }

        $newConfig = collect(self::$defaultQueryBuilderConfig)->map(function ($value) use ($name) {
            return "{$name}_{$value}";
        })->all();
     ...
     *
     * @param \Inertia\Response|\App\Providers\InertiaTableServiceProvider $response
     * @return \Inertia\Response
     */
    public function applyTo(Response|\App\Providers\InertiaTableServiceProvider $response): Response
    {
